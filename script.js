 ...
 * @returns {CombinationManager}
 */
function initword(str,words){
    const manager = new CombinationManager(str)
    const chunkSize = 5000;
    for (let i = 0; i < words.length; i += chunkSize) {
        const chunk = words.slice(i, i + chunkSize);
        manager.add_words(chunk);
    }
...
function makedata(manager1) {
    ...
    //console.log(result6);
    const manager2=initword(manager1.remainstr(),dictionary5)
...
function processing(mode, str) {
    const manager=initword(str,dictionary6);
